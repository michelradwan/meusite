...
# 1. Update .browser-reveal-section CSS
...
# 2. Update updateArch JS to output window.scrubProgress
scrub_target = 'const easeOut = 1 - Math.pow(1 - progress, 3);'
scrub_replacement = '''window.scrubProgress = progress; // Expose for particles warp speed
      const easeOut = 1 - Math.pow(1 - progress, 3);'''
content = content.replace(scrub_target, scrub_replacement)

# 3. Modify Particles Canvas logic
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print('Applied warp speed particles!')
